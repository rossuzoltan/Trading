...
class Mt5CursorTickSource:
    ...
    def fetch(self, symbol: str, cursor: TickCursor) -> tuple[list[TickEvent], TickCursor]:
        ...
        # MT5 tick records on this broker are stamped in server-local time, not true UTC.
        # The cursor is maintained in normalized UTC, so request windows must be shifted
        # forward into broker-local time before calling copy_ticks_from().
